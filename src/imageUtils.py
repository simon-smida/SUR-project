...
def cross_validation(dataset, num_splits=NS):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(dataset)):
        ...
        model = CNN(num_classes=1, cnn_dropout=CNN_DROPOUT_RATE, fc_dropout=FC_DROPOUT_RATE).to(device)
        # Train the model
...
def predict(model, image_tensor):
    ...
    with torch.no_grad():
        # NOTE: Use sigmoid activation for BCEWithLogitsLoss
        #logits = model(image_tensor)
        #probabilities = torch.sigmoid(logits)  # Convert logits to probabilities
        # NOTE: Oversampling with BCELoss
